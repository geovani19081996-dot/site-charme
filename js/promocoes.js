...
//  - Carrega JSON local/publicado, aplica filtros, paginação e timers
// =======================================================
...
(function () {
  ...
  async function fetchPromoJson() {
    ...
    try {
      const response = await fetch(PROMOS_JSON_URL, {
        cache: "no-store",
  ...
  const boot = () => {
    ...
    const loadPromos = async () => {
      try {
        const data = await fetchPromoJson();
